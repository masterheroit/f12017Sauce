...
inline void callDitto(std::string path, std::string out)
{
	// Read file
	fstream argFile;
	argFile.open(path.c_str(), ios_base::in | ios_base::binary | ios_base::ate);
	if(!argFile.is_open())
		exitWithError();
	size_t dataSize = argFile.tellg();
	uint8_t* data = new uint8_t[ dataSize + 2048 ];
	...
	**/
	...
	Transform* trans;
	try {
		trans = new Transform(*disasm, *parser, 66); // The ctor performs the analysis
	}
	catch(const char* e) {
		exitWithError(string("FAIL (") + e + ")\nAborting.\n");
	}
	...
	**/
	...
	fstream outFile;
	outFile.open(out.c_str(), ios_base::out | ios_base::binary | ios_base::trunc);
	if(!outFile.is_open())
