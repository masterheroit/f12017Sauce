...
// returns time
//inline float GetConstantViewLag()
//{
//	// Get true latency
//
...
//	// HOPE THAT IT IS THE DEFAULT VALUE
//	int lerpTicks = TIME_TO_TICKS( 0.1 );
//
...
// returns a negative number that is the difference from globals->curtime
inline float GetActualLerpDiff()
{
	float flLerp = GetConstantLerp();
	return flLerp - TICKS_TO_TIME(TIME_TO_TICKS(flLerp));
}
...
static void RestorePlayerTo( CBaseEntity *pPlayer, const Vector &vWantedPos )
{
	...
	if( tr.startsolid || tr.allsolid )
	{
		...
		UTIL_TraceEntity( pPlayer, pPlayer->GetAbsOrigin(), vWantedPos, MASK_PLAYERSOLID, pPlayer, COLLISION_GROUP_PLAYER_MOVEMENT, &tr );
		if( tr.startsolid || tr.allsolid )
		{
		...
		else
		{
			...
			// TODO do we need to set the origin?
			// TODO change CBaseEntity to abstract this please
...
public:
	...
	// Called during player movement to set up/restore after lag compensation
	void			StartLagCompensation( CBaseEntity *player, CUserCmd *cmd );
...
void CLagCompensationManager::FrameUpdatePostEntityThink()
{
	...
	for( int i = 1; i <= gInts->Globals->maxclients; i++ )
	{
		...
		if( track->Count() > 0 )
		{
			...
			if( head.m_flSimulationTime >= pPlayer.get<float>(gEntVars.flSimulationTime) )
		...
		record.m_fFlags = 0;
		if( pPlayer.get<BYTE>(gEntVars.iLifeState) == LIFE_ALIVE )
		{
			record.m_fFlags |= LC_ALIVE;
		}

		record.m_flSimulationTime = pPlayer.get<float>( gEntVars.flSimulationTime );
		record.m_vecAngles = pPlayer->GetPrevLocalAngles();
		...
		// TODO dont hardcode
		record.m_vecMinsPreScaled = pPlayer.get<Vector>( gEntVars.collision + 0x8 );
		record.m_vecMaxsPreScaled = pPlayer.get<Vector>( gEntVars.collision + 0x14 );
...
void CLagCompensationManager::BacktrackPlayer( CBaseEntity *pPlayer, float flTargetTime )
{
	Vector org;
	...
	if( sv_unlag_fixstuck.GetBool() )
	{
		...
		if( tr.startsolid || tr.allsolid )
		{
			...
			CBaseEntity *pHitPlayer = dynamic_cast<CBaseEntity *>( tr.m_pEnt );
