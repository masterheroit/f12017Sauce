...
enum network_ehandle
{
	// How many bits to use to encode an edict.
	MAX_EDICT_BITS = 11,					   // # of bits needed to represent max edicts
	MAX_EDICTS = (1 << MAX_EDICT_BITS), // Max # of edicts in a level
...
public:
	...
	bool operator ==(const CBaseHandle &other) const;
	bool operator ==(const IHandleEntity* pEnt);
	bool operator !=(const IHandleEntity* pEnt);
	bool operator <(const CBaseHandle &other) const;
	bool operator <(IHandleEntity* pEnt) const;

	// Assign a value to the handle.
	const CBaseHandle& operator=(IHandleEntity *pEntity);
	const CBaseHandle& Set(IHandleEntity *pEntity);
...
inline bool CBaseHandle::operator ==(const IHandleEntity* pEnt)
{
	return Get() == pEnt;
}
...
inline bool CBaseHandle::operator <(IHandleEntity *pEntity) const
{
	unsigned long otherIndex = (pEntity) ? pEntity->GetRefEHandle().m_Index : INVALID_EHANDLE_INDEX;
	return m_Index < otherIndex;
}

inline const CBaseHandle& CBaseHandle::operator=(IHandleEntity *pEntity)
{
	return Set(pEntity);
}

inline const CBaseHandle& CBaseHandle::Set(IHandleEntity *pEntity)
{
	if(pEntity)
	{
